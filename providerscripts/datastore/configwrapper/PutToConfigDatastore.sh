...
then
        ...
        datastore_cmd="${datastore_tool} --credentials-file /root/.s5cfg-1 --endpoint-url https://${host_base} cp  --metadata 'CreationDate=`/usr/bin/date`'"
        bucket_prefix="s3://"
        if ( [ "${place_to_put}" = "" ] )
        then
                slasher=""
...
then
        ...
        ${datastore_cmd1} ${bucket_prefix}${config_bucket}${place_to_put}${slasher}${placed_file}
fi
