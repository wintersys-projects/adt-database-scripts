...
then
    ...
    then
        CLOUDHOST="`${HOME}/providerscripts/utilities/config/config/ExtractConfigValue.sh 'CLOUDHOST'`"
        ...
        then
             ...
             then
                 ...
                 then
                     /usr/bin/curl -sSL https://repos.insights.digitalocean.com/install.sh | /usr/bin/sudo bash   #####UBUNTU-DIGITALOCEANMONITORING-REPO#####
                 fi
            fi
            if ( [ "${BUILDOS}" = "debian" ] )
            then
