...
then
	...
	if ( [ "${cpu_usage}" -lt "25" ] )
	then
		${HOME}/providerscripts/datastore/config/wrapper/PutToDatastore.sh "config" "${ip}" "overloadedips" "local" "no"
	else
		${HOME}/providerscripts/datastore/config/wrapper/DeleteFromDatastore.sh "config"  "overloadedips/*" "local" 
	fi
