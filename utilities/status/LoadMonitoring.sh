...
then
	...
	then
		${HOME}/providerscripts/email/SendEmail.sh "HIGH PERCENTAGE LOAD" "More than a quarter of samples had more than 90% load over the past hour on machine with IP address `${HOME}/utilities/processing/GetPublicIP.sh`" "ERROR"
	fi
	/bin/rm ${HOME}/runtime/ATOP_RUNNING
fi
